...
motor_left = Motor(
    in1=LEFT_MOTOR_IN1, in2=LEFT_MOTOR_IN2, ena=LEFT_MOTOR_ENA, is_left_motor=True
)
motor_right = Motor(
    in1=RIGHT_MOTOR_IN1, in2=RIGHT_MOTOR_IN2, ena=RIGHT_MOTOR_ENA, is_left_motor=False
)

# setup weapon
led = machine.Pin(ONBOARD_LED_PIN, machine.Pin.OUT)
# Reuse the existing servo instance from boot.py rather than creating a new one
try:
    import sys
    weapon_servo = sys.modules.get('__main__').weapon_servo
except:
    # Fallback if we can't get the existing servo instance
    weapon_servo = machine.PWM(machine.Pin(WEAPON1_PIN), freq=50)
    # Don't move the servo on initialization
        ...
        if path == "/status":
            ...
            if request_type == "GET":
                ...
                try:
                    ...
                    if (
                        "on" not in settings["servo1"]
                        or "off" not in settings["servo1"]
                    ):
